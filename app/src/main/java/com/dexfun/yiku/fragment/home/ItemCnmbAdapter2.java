...
public class ItemCnmbAdapter2 extends PagerAdapter {
    private boolean isVip = SharedPreferencesUtil.getInstance().getBoolean("isVIP", false);
    Context context;
    List<List<HOME_ENTITY.DataEntity.OrnamentsEntity.ContentEntityX>> mmpList = new ArrayList<>();
    private String token = UserClass.getInstance().getToken();

    public ItemCnmbAdapter2(Context context, List<List<HOME_ENTITY.DataEntity.OrnamentsEntity.ContentEntityX>> mmpList) {
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        HOME_ENTITY.DataEntity.OrnamentsEntity.ContentEntityX contentEntity = mmpList.get(position).get(0);
        HOME_ENTITY.DataEntity.OrnamentsEntity.ContentEntityX contentEntity1 = mmpList.get(position).get(1);
