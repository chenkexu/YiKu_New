...
public class UiUtils {
    ...
    public static void setFitsSystemWindows(Activity context, boolean enabled) {
        if (enabled) {
            return;
        }
        ViewGroup content = context.findViewById(android.R.id.content);
        View baseView = content.getChildAt(0);
        if (baseView != null) {
            baseView.setFitsSystemWindows(true);
            baseView.setBackgroundResource(R.color.white);
            baseView.setOnClickListener(view -> inIvisibleInput(context, baseView));
...
/*    public static void setIndicator(TabLayout tabs, int leftDip, int rightDip) {
    ...
    public static void toastError() {
        Toast.makeText(MainClass.getInstance().getApplicationContext(), "Network error", Toast.LENGTH_SHORT).show();
    }
