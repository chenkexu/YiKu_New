...
public class WeichatShareImpl extends BaseShareWay {
    ...
    public void shareToWeixin(final String title, final String content, final String imgUrl, final boolean friend, final String shareUrl) {
        if (TextUtils.isEmpty(title)) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    Bitmap bitmap = BitmapFactory.decodeResource(activity.getResources(), R.drawable.share_card);
                    WXImageObject object = new WXImageObject(bitmap);
                    WXMediaMessage msg = new WXMediaMessage(object);
                    Bitmap bitmap1 = BitmapFactory.decodeResource(activity.getResources(), R.drawable.ic_launcher);
                    msg.thumbData = com.tencent.mm.sdk.platformtools.Util.bmpToByteArray(bitmap1, true);
